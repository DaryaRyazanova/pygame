...
class Life(Board):
    def __init__(self, width=10, height=10):
        ...
        self.board = [[1 if random.random() < 0.2 else 0 for _ in range(width)] for _ in range (height)]
            ...
            for col in range(self.width):
                ...
                for d in dx:
                    if (0 <= row + d[0] < self.height and 0 <= col + d[1] < self.width and
                            self.board[row + d[0]][col + d[1]] == 1):
                        s += 1
                if self.board[row][col] == 1 and 2 <= s <= 3 or self.board[row][col] == 0 and s == 3:
    ...
    while running:
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if event.button == 4:  # ускорить
                    if fps < 40:
                        fps += 1
                if event.button == 5:  # замедлить
                    if fps > 1:
                        fps -= 1
